...
const Sidebar = ({ isOpen, onClose, user, isAuthenticated }) => {
  ...
  const NavLink = ({ item }) => (
    ...
    >
      <span className={isActive(item.href) ? 'text-blue-500 dark:text-blue-400' : 'text-gray-400 dark:text-slate-500 group-hover:text-gray-500 dark:group-hover:text-slate-300'}>
        {item.icon}
      </span>
      {item.name}
  ...
  const Content = () => (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900">
      <div className="flex items-center justify-between h-16 px-5 border-b border-gray-200 dark:border-slate-700 flex-shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center flex-shrink-0">
            <span className="text-white font-bold text-sm">UH</span>
          </div>
          <span className="text-xl font-bold text-gray-900 dark:text-white">UniHub</span>
        </div>
        <button onClick={onClose} className="lg:hidden p-1.5 rounded-md text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-800" aria-label="Close menu">
      ...
      <nav className="flex-1 px-3 py-4 space-y-5 overflow-y-auto">
        <div>
          <p className="px-3 mb-2 text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider">Main</p>
          <div className="space-y-0.5">
            {NAV_ITEMS.map((item) => <NavLink key={item.name} item={item} />)}
          </div>
        </div>
        <div>
          <p className="px-3 mb-2 text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider">Account</p>
          <div className="space-y-0.5">
      ...
      <div className="px-5 py-3 border-t border-gray-200 dark:border-slate-700 flex-shrink-0">
        <p className="text-xs text-gray-400 dark:text-slate-500">© 2024 UniHub · v1.0.0</p>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:flex-shrink-0 bg-white dark:bg-slate-900 border-r border-gray-200 dark:border-slate-700 h-screen sticky top-0">
        <Content />
      </aside>

      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-slate-900 shadow-xl transform transition-transform duration-300 ease-in-out lg:hidden ${
